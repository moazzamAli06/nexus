...
function($scope,$rootScope,$location,profileService,$http,moment,socialLoginService) {
	...
	$scope.timezone = {
		model:null,
		availableOptions:	[
	  { "timeZoneId":"1" ,"gmtAdjustment":"GMT-12:00" ,"useDaylightTime":"0", "value":"-12", "title" :"(GMT-12:00) International Date Line West"},
	...
	$scope.get = function(){
		...
		url: 'profile/show'
			...
			$scope.liveIncountries.model = response.data.country;
			$scope.google_id = response.data.google_id;
			$scope.long_info = response.data.long_info;
			$scope.short_info = response.data.short_info;
			$scope.month.model = response.data.dob.split('/')[0];
			$scope.day.model = response.data.dob.split('/')[1];
			$scope.year.model = response.data.dob.split('/')[2];
			$scope.qq = response.data.qq;
			$scope.skype_id = response.data.skype_id;
			$scope.face_time = response.data.face_time;
			$scope.city = "lahore";//response.data.city;
			var day = response.data.created_at.split('-')[0];
